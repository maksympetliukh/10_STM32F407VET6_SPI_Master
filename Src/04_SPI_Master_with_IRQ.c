...
void SPI_ApplicationEventCallback(SPI_Handle_t *pHandle, uint8_t event_flag){
	if(event_flag == SPI_EVENT_TX_CMPLT){
		if(!tx_done){
		tx_done = 1;

		SPI_Transmit_IT(pHandle, &dummy, 1);
	}else{
		SPI_Receive_IT(pHandle, &rx, 1);
	}
	}else if(event_flag == SPI_EVENT_RX_CMPLT){
		tx_done = 0;
	...
	}else if(event_flag == SPI_EVENT_OVR_ERR){
